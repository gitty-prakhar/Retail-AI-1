...
function getRisk(d){if(d<=3)return"HIGH";if(d<=7)return"MEDIUM";return"SAFE";}
let filter="ALL";

function render(){
 const tb=document.getElementById("invBody");tb.innerHTML="";
 products.filter(p=>filter=="ALL"||getRisk(p.daysLeft)==filter)
 .forEach(p=>{
  const r=getRisk(p.daysLeft);
  tb.innerHTML+=`
   <tr>
    <td>${p.name}</td><td>${p.category}</td><td>${p.daysLeft}</td>
    <td><span class="risk ${r.toLowerCase()}">${r}</span></td>
   </tr>`;
 });
}
document.querySelectorAll(".f").forEach(b=>{
 b.onclick=()=>{document.querySelectorAll(".f").forEach(x=>x.classList.remove("active"));
